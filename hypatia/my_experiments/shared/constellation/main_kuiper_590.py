...
BASE_NAME = "kuiper_590"
NICE_NAME = "Kuiper-590"
...
# 轨道参数 [1]
MEAN_MOTION_REV_PER_DAY = 14.95  # 平均运动 (圈/天) — 高度 ~590 km
ALTITUDE_M = 590000              # 轨道高度 (m)

# 覆盖范围: 56°N ~ 56°S
# 最小仰角取 30°（Kuiper 各壳层统一值）
...
NUM_ORBS = 28                    # 轨道面数
NUM_SATS_PER_ORB = 28            # 每面卫星数
INCLINATION_DEGREE = 33.0        # 倾角 (°)
...
def main():
    """命令行入口: python main_kuiper_590.py <时长s> <步长ms> <ISL模式> <地面站> <路由算法> <线程数>"""
    args = sys.argv[1:]
    if len(args) != 6:
        print("必须提供 6 个参数")
        print("用法: python main_kuiper_590.py [时长(s)] [步长(ms)] "
