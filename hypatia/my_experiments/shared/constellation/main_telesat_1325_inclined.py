"""
Telesat Lightspeed Final Constellation — 倾斜壳

轨道参数来源：FCC SAT-MPL-20200526-00053; Table A.2-1; ISED/ITU: CANSAT-LEO

高度 1325 km, 倾角 50.88°, 40 轨道面 × 33 星/面, 共 1320 颗卫星
面内相位间隔 ≈ 10.91°, 最低仰角 10°
"""
import math
import sys
...
BASE_NAME = "telesat_1325_inclined"
NICE_NAME = "Telesat-1325-Inclined"
...
# 轨道参数 [1]
MEAN_MOTION_REV_PER_DAY = 12.86  # 平均运动 (圈/天) — 高度 ~1325 km
ALTITUDE_M = 1325000             # 轨道高度 (m)
...
NUM_ORBS = 40                    # 轨道面数
NUM_SATS_PER_ORB = 33            # 每面卫星数
INCLINATION_DEGREE = 50.88       # 倾角 (°)
...
def main():
    """命令行入口: python main_telesat_1325_inclined.py <时长s> <步长ms> <ISL模式> <地面站> <路由算法> <线程数>"""
    args = sys.argv[1:]
    if len(args) != 6:
        print("必须提供 6 个参数")
        print("用法: python main_telesat_1325_inclined.py [时长(s)] [步长(ms)] "
