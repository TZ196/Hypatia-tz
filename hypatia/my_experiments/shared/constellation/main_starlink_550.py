"""
Starlink SpaceX Phase 1 Gen1 — Shell 1

轨道参数来源：FCC 21-48; FCC 19-67; SAT-MOD-20190830-00087
    最低仰角 25° 来自 FCC SAT-MOD-20181108-00083

高度 550 km, 倾角 53°, 72 轨道面 × 22 星/面, 共 1584 颗卫星
"""
import math
import sys
...
# 轨道参数 [1]
MEAN_MOTION_REV_PER_DAY = 15.19  # 平均运动 (圈/天) — 高度 ~550 km
ALTITUDE_M = 550000              # 轨道高度 (m)

# 地面圆锥半径来自 FCC SAT-MOD-20181108-00083/1569860.pdf (最低仰角 25°)
...
def main():
    """命令行入口: python main_starlink_550.py <时长s> <步长ms> <ISL模式> <地面站> <路由算法> <线程数>"""
    args = sys.argv[1:]
    if len(args) != 6:
        print("必须提供 6 个参数")
        print("用法: python main_starlink_550.py [时长(s)] [步长(ms)] "
