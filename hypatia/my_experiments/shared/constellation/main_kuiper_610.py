...
# 轨道参数 [1]
MEAN_MOTION_REV_PER_DAY = 14.89  # 平均运动 (圈/天) — 高度 ~610 km
ALTITUDE_M = 610000              # 轨道高度 (m)
...
NUM_ORBS = 36                    # 轨道面数
NUM_SATS_PER_ORB = 36            # 每面卫星数
INCLINATION_DEGREE = 42.0        # 倾角 (°)
...
def main():
    """命令行入口: python main_kuiper_610.py <时长s> <步长ms> <ISL模式> <地面站> <路由算法> <线程数>"""
    args = sys.argv[1:]
    if len(args) != 6:
        print("必须提供 6 个参数")
        print("用法: python main_kuiper_610.py [时长(s)] [步长(ms)] "
